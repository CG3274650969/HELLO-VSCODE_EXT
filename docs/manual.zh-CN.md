...
- **转写里多一条说明**（「上下文已压缩：前 N 条事件（约 X token）已折叠成摘要…」），用量条上多一个
  计数（**已压缩 N 次** —— 说明会滚走，计数不会）。压缩失败也会说，并说明记忆**没有被动过**。摘要
  正文**有意不**回显进转写 —— 那等于把一份二手回忆倒进你的记录里。
- **用量条带上占用值**，接在原有读数后面：`上下文 24.0K / 1.0M`；接近那条线时加
  `⚠ 接近压缩阈值`，而数字是上一次运行的最后一个样本、不是实时值时加 `· 上次`。
  ⚠️ **两个数不是同一个数。** 条上的占用是 provider 上报的提示侧压力；DSH 决定要不要压缩用的是它
