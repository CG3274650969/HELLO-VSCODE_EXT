...
The key only ever goes into the child process env (plus the two read-only balance/model requests the
extension makes itself) — never into settings, logs or the transcript.
...
### Account balance: `余额：¥28.70`

The chip sitting next to the **API** button on the config bar is your DeepSeek account balance.
**Click it to re-query** (hover for the details: total / granted / topped-up per currency, and the
clock time that number was fetched).

- **Four states, all of them words rather than graphics.** No key configured ⇒ `余额：—` (the tooltip
  tells you to click **API**); first query in flight ⇒ `余额：…`; we got it ⇒ `余额：¥28.70`; the
  account is suspended ⇒ `余额：不可用` (`is_available = false`). **A currency we do not recognise is
  written `28.70 XYZ` — we never guess one for you**; with several currencies only the first goes on
  the button, the rest are all listed in the tooltip.
- **When it refreshes on its own:** once when the harness connects, **once when you start a new
  conversation**, and then **every 5 minutes**. It also fetches once right after you configure a key.
- **A failed fetch never erases a number you already have**: the last good value stays (dimmed), and
  the tooltip says which failure it was plus when that value was fetched. **Stale and live have to be
  distinguishable at a glance**, or what you are holding is a number of unknown age — the same rule as
...
Apart from the trailing 「自定义模型…」 entry, the model menu lists **the models your account can
currently use** (the `GET /models` response) instead of our built-in presets. **If that call fails we
fall back to the presets** (no key / offline / 401 / shape changed all count as failure) — the menu
still works and is never empty. The model you are using **is always in the list**; if it is not, it is
put first.

> Requests go only to two **read-only** GETs on `https://api.deepseek.com` (`/user/balance`, `/models`)
> with `Authorization: Bearer <key>`. The key **only ever goes into the request header**: never into
> logs, transcripts, settings, or any error text (a server response body is stripped of anything
> key-shaped and truncated before it reaches a tooltip). The target host is **hard-coded** with no
> configuration entry — changing it means changing the code.
...
| `src/deepseekApi.ts` | All of C23's **judgement**: parsing what the API returns (amounts arrive as strings, `is_available` is three-valued, one dirty element is skipped rather than fatal), formatting a balance into the button text and the tooltip detail, turning an error kind into plain language, `sanitize()` scrubbing key-shaped fragments, and **the one thin HTTP layer that makes an outbound request** (`fetchJson()` — the key is composed into `Authorization` only inside it and no header is ever returned, so no caller gets the chance to paste it into an error message). **Does not import `vscode`**: nearly every line here is "what to do when we cannot get this, or get something strange", and only a probe can walk those branches repeatedly (pure functions + an injectable transport, no `vscode`) |
...
- The key flows to exactly two places: the child-process env (`DEEPSEEK_API_KEY`), and two
  **read-only** GETs this extension makes itself to `api.deepseek.com` (`/user/balance`, `/models`)
  with `Authorization: Bearer`. It must never appear in logs, transcripts, settings, or this repo —
  and never in any error text either: the header is composed inside one function, never returned to
  callers, and any server response body is stripped of key-shaped fragments before it is shown.
