...
| C21 | composer 读数条合并：**读数不画卡，只有「要你动手」的才配一张卡** | P2 | **不是新功能，是外观收口**：真机上 composer 同时叠着四条（附件 chip 行 / 用量条 / 运行条 / 「继续」条），四条用的是**同一张软卡配方**（1px 边框 + 圆角 + 底色 + 11px 灰字）⇒ **读数和动作一样重**，四张卡并排像表格行，而 `.near` 那个警示色**淹没在同色卡里喊不出来**。改法（用户 2026-09-24 拍板）：**usage + runs 合成一条无边框无底色的读数行 `#status-row`**（左半 usage、右半 runs、最右是默认淡出的「运行记录 ›」），**动作条（继续 / 审批 / 审阅）保留卡** —— 卡的数量从此与「要不要你动手」一一对应；`.near` 成为**全 composer 唯一**会变色/给底色的东西（⚠️ **C22 起这一处从读数行换到了配置条上的一枚环** —— 判据本身不变，只是换了地方）。**已实现（2026-09-24）**：`probe-webview-render`（C21 落笔时 104/104；**C22 之后是 112/112**），6 条变异全被抓红；**F5 八条待跑**。**⚠️ 同日 C21b**：「运行记录 ›」入口原来是 `opacity: 0`（不悬停就完全看不见），**真机一跑用户就否了**（「和背景融为一体了」）⇒ 改成 **`0.55` 半淡常显**（悬停/聚焦推满），守卫从"含 `opacity: 0`"改成"默认落在 `(0,1)` 区间"，3 条变异全红；`media/chat.html` 与 `chat.js` 一个字节没动，见正文 C21b | [~] |
| C22 | 上下文占用改成配置条上的一枚**小圆环**（模型钮与推理档位钮之间） | P2 | **不是新功能，是读数换形态**：用户 2026-09-24 提出「`累计 ↑1.1M ↓57.8K · 上下文 76.9K / 1.0M（8%）` 这段能不能做成像 Claude Code 那样、在模型版本选择和推理挡位之间的一个小圆环，表示上下文占用比例，鼠标放上去有完整悬浮信息」。拍板四条：① 读数行左半**完全由圆环取代**（本轮/累计/压缩次数全进悬浮信息）；② 圆环**纯读数、不可点**；③ `.near` 警示色**搬到环上**（读数行不再变色）；④ `stale` **环变淡 + 提示里说明**。**已实现（2026-09-24）**：`probe-webview-render` **112/112**（原 104 + 8），**7 条变异全被抓红**；**F5 八条待跑**（⚠️ **与 C22b 的 F5 合并跑一次**：同一条工具栏、同一个控件，环现在还能点开浮层）。⚠️ **顺带逮到一颗真雷**：`.near` 用的 `--dsw-alias-state-**warning**-primary` 在 `dsh-live.css` 里**根本没定义**（那里只有 `warn`）⇒ 一直静默退到 `--vscode-editorWarning-foreground` 兜底色，看着"差不多对"所以没人发现。**C22 只修了环上这一处**；同款写法在 `chat.css` 里**还有 8 处**（C21 之前就在），**未修**，见正文。**⚠️ 同日 C22b 推翻第 ② 条**：真机看过之后用户要求「变成一个按钮 + 用推理/profile 相同的容器」⇒ 环成了配置条第三个方块钮、点开一个只读浮层（`probe-webview-render` **121/121**，8 条变异全红），见正文 C22b。**⚠️ 同日 C22c 又推翻一处**：一条用量都拼不出来时**不再藏环**（新建对话里也看得见它，只是弧空、读作 0 —— 但不写数字，见正文 C22c），并把浮层排版整个重做；`probe-webview-render` **123/123**，13 条变异全红。**⚠️ 同日 C22d 再修一处**：浮层里那两行口径说明用的是 `--dsw-alias-label-dimmed` —— 那是这套令牌梯里**最淡的一档**（暗色 = bluish-750 `rgb(67,69,74)`，而浮层底 ≈`rgb(43,43,48)`，≈1.5:1），用户原话「黑色主题下有点暗」⇒ 换 `--dsw-alias-label-tertiary`（层级仍是 数字 > 行标签 > 说明），`probe-webview-render` **124/124**、4 条变异全红，见正文 C22d | [~] |

| C23 | 余额读数 + **模型列表改由远端驱动** | P2 | **由来（用户 2026-09-24，附了两份 Python 脚本）**：「这两个地址分别查询余额和模型列表，我要将其集成到对话中，我要在"API"按钮旁加一个"余额：xxx"的按钮，然后将模型列表的除自定义之外的其他模型选项改为显示模型列表里的内容」—— 即 `GET /user/balance` 与 `GET /models`。**这是本仓第一次让扩展自己往外发请求**（此前 `src/` 里连 `fetch` 都没有），所以除功能之外还立了两条**结构上**的规矩：key 只进请求头（由 `fetchJson` 内部拼、返回值里没有 header，调用方无从"顺手拼进错误信息"）、服务端正文进任何文案之前先过 `sanitize()`（**它可能把 key 回显出来**）。四条拍板：① 余额钮**点了就刷新**（明细进 `title`）；② 联网时机 = **连接时 / 新建对话时 / 之后每 5 分钟**（配好 key 后补取一次）；③ 取不到远端列表 ⇒ **退回本地预设**；④ 余额字面 `余额：¥28.70`（CNY→`¥`、USD→`$`，认不出写 `28.70 XYZ`，多币种取第一个、`title` 里列全）。**已实现（2026-09-24）**：判断全在不 import `vscode` 的新模块 `src/deepseekApi.ts` 里，探针 `probe-deepseek-api` **38/38**（含"服务器把 key 回显进错误正文"那条）、`probe-webview-render` 124 → **132/132**，**9 条变异全红且各打红该红的那条**；**F5 八条待跑**。⚠️ **5 分钟轮询是这一轮唯一"自己会跑"的东西**，所以给它立了**三条闸**：没 key 一个请求都不发、上一发没回来不叠发、关面板/切走 harness 就停表 —— 缺一条就是"后台静默发请求" | [~] |
> **C18 已删（2026-09-23，用户决定不做）** —— 原为「企业集成：代理 / 远程开发 / 审计日志」。编号**空着不补**：C19 / C20 在提交历史、正文与外部记录里已经用这两个号叫开了，重排只会让历史对不上号。

> **2026-09-24 · 上线前藏起「内嵌聊天」入口（注释，不是删）** —— `media/chat.html` 里那条 `#mode-bar` 整块注释掉；扩展侧加 `CHAT_MODE_EXPOSED`（`src/chatViewProvider.ts`）挡住**陈旧的 globalState 值** —— 用户此前选过内嵌聊天的话，没有这道闸会启动即进 chat、而入口已经藏了 ⇒ 再也切不回来。**放开 = 去掉 HTML 注释 + 把那个常量改回 `true`，两件都要做**：常量刻意只闸「读」、不闸 `set-mode`，只做一半会表现为按钮点了没反应（静默无效）。两向守卫在 `probe-webview-render`（剥掉注释后必须找不到、原文里必须还在）。
...
- **用户拍板（2026-09-18）**：四件都做 —— ① 占用指示 + 接近上限提示；② 把 DSH 的压缩变成可见；③ 调低阈值让它真的会发生；④ 折叠我们自己的转写（旧轮）。提示落在 **`#usage-bar`**（不新开条/浮层）。阈值**做成设置项、默认仍 0.8**（零行为改变）。（⚠️ **2026-09-24 C21 起那个 id 没了**：用量条与 C9 运行条合并成一条**读数行** `#status-row`，这条占用读数现在是它的左半 —— **「不新开条/浮层」这个判断一个字没变**，变的只是那条不再画卡，见 C21。）
- **D1 · 占用指示（扩 `#usage-bar`）**：数据与 UI 位都已有了，缺的只是「接近上限喊一声」。
  ...
  - （**搬过两次家，判据一次都没动**：**C21 起**这段逻辑所在的函数叫 `paintUsage()`、文案写在 `#status-summary` 上、`.near` 挂在 `#status-row` 上（「条」变成「行」）；**C22 起**函数叫 `paintContextRing()`、文案写在 `#ctx-ring` 的 `title` 上、`.near` 挂在环上（「行」变成「配置条上的一枚环」）。上面这几条判据 —— 两个后缀并列、`near`/`stale` 各自独立、`pct >= 1` 才显示百分比、口径必须诚实 —— **一个字都没改**。`.near` 至今仍是**全 composer 唯一**会变色的地方。）
...
- **本次不做**：不按需触发压缩（wire 没口子）；不改 DSH 的 `session.jsonl.zstd`；不新开 `#context-bar` / 第三个浮层（用户选了扩 `#usage-bar` —— **C21 起 `#usage-bar` 这个 id 已换成读数行 `#status-row` 的左半，但「不新开浮层」这条判断不变**）；不做「把旧消息真的折成一段摘要文本」（要动存储，且那是 DSH 那侧的事）；不改 `token-meter`（它没有任何可配置项，加任何键都会加载失败）。
- **已知局限（要留着）**：
...
### C21 · composer 读数条合并：读数不画卡，只有「要你动手」的才配一张卡（2026-09-24 实现，F5 八条待跑）
- **由来**：用户 2026-09-24 贴了一张真机截图 —— composer 上方**同时**叠着四样东西：附件 chip 行、用量条、运行条、「继续」条。视觉问题是**根子上的**：这四条用的是**同一张软卡配方**（`media/chat.css` 里 `.usage-bar` / `.runs-bar` / `.retry-bar` / `.review-bar` 逐字同款：1px 边框 + `border-radius: 10px` + `--dsw-alias-layer-1` 底 + 11px 灰字）⇒ **读数和动作一样重**，四张卡并排像表格行，谁都在争注意力。
- **判定（一句话）**：**读数不画卡，只有「要你动手」的才配一张卡。** 卡的数量从此与「要不要你动手」一一对应：审批 / 审阅 / 继续三条是**动作**（要你点东西），本来就该有表面；usage / runs 是**读数**，不该有。
  - **顺手把「跳色」变成稀缺资源**：四张卡同一个灰底，`--dsw-alias-state-warning-primary` 那套 `.near` 告警淹没在里面。改成**只有 `.near` 能让 composer 变色/给底色**之后，「上下文要压了」才真喊得出来。这条是**这次改动最值钱的一半**，也是最容易被后人当"顺手美化"改回去的一半。
    - ⚠️ **C22（次日）把这一处从"读数行"换到了"配置条上的一枚环"** —— 「全 composer 只有一处会变色」这条**判据一个字没变**，只是那处换了个节点。**别读到 C21 这一行以为警示色还在 `#status-row` 上**；C22 同时删掉了 `.status-row.near` 并加了一条守卫（同一个警示不许在两处一起喊）。
    - ⚠️ 也就在 C22 才发现：C21 在这里写的 `--dsw-alias-state-warning-primary` **是颗哑炮** —— 那个名字在 `dsh-live.css` 里根本没定义，它一直静默退到 `--vscode-editorWarning-foreground` 兜底色。**"看起来是对的"掩盖了"它压根没生效"**，参见 C22。
- **落点（三个文件，`src/` 一个字节没动）**：
  - [media/chat.html](../media/chat.html)：`#usage-bar` + `#runs-bar` 两块（含各自注释）合并成一个 `#status-row`（`hidden`，内含 `#status-summary` / `#status-run` / `#runs-view`）。
    - ⚠️ **`#runs-view` 这个 id 一个字不动** —— 点击绑定与探针的 C9/C15 检查都按它找元素，改 id 是无谓的连带改动；变的只是它挂上了 `.status-open`、文案从「查看」改成「运行记录 ›」。
    - ⚠️ 两处「挂在 composer 内（react-live 只藏 `#messages`）」的约束照旧成立 —— 读数行**不能**挪进消息区尾部。
  - [media/chat.css](../media/chat.css)：删掉两张卡的四组规则（`.usage-bar` / `.usage-summary` / `.usage-bar.near` / `.runs-bar` / `.runs-summary`），加一条 `.status-row`（**没有 `border`、没有 `background`** —— 这就是「读数不画卡」本身）+ `.status-summary` / `.status-run`（各自省略号截断 + 等宽数字防抖动，原注释逐字保留）+ `.status-open` + `.status-row.near`。
    - **`.near` 只换配色、不动尺寸**：行高一变，流式刷新时 composer 会跳（阈值附近来回摆动时最容易看出来）。原来那两句注释照留。
  - [media/chat.js](../media/chat.js)：`renderUsageBar()` + `renderRunsBar()` → **一个 `renderStatusRow()`**，它是这一行 `hidden` 与 `.near` 的**唯一写入者**（两个函数各写一次迟早互相盖 —— C10b 那一型的形状）。左半 `paintUsage()` / 右半 `paintRuns()` 各写自己的文案与 title、各返回「有没有内容」，**都不碰 `hidden`**。
- **显隐判据：两半合起来算**（`usageState || runsState`），这**恰好**保住两条老行为，改的时候别当巧合：
  - **harness 下 `_runsReadout()` 恒返回读数**（从没跑过也是 `{runs: []}`，`chatViewProvider.ts` 那处注释就是为这条写的）⇒ 空读数写「本轮尚无」、**行照样显示**。这是 C9 的老决定：**入口不能等第一轮跑完才出现**。
  - 内嵌聊天两半都是 `undefined`（`_usageReadout` 守卫）⇒ 整行隐藏。
- **⚠️ 两个字段名不能统一**：快照那条带 `data.runs`，流式 `runs` 消息带 `data.readout`。合并渲染函数时最顺手的"清理"就是把它俩统一 —— 那会**静默丢掉一半数据**（只有一条路还能画）。两条路各有探针钉着。
- **⚠️ `.status-open` 用 `opacity` 而不是 `display:none` / `visibility:hidden`**，两个理由缺一不可：
  - `opacity` 不把元素摘出流 ⇒ 按钮宽度**恒占位** ⇒ 鼠标移入移出时左边的字**不重排**（用 `display` 换显隐会让整行抖一下）；
  - 不透明度**不影响可聚焦**，键盘 Tab 到它的那一刻 `.status-row:focus-within` 就把它推到满 —— 一个 `display:none` 的入口键盘永远够不到。**这是判据，不是好意**：探针里有 CSS 守卫钉着（不许出现 `display:none` / `visibility:hidden`、必须有一条 `:focus-within` 显形规则、且默认不透明度要落在 `(0, 1)` 开区间里）。触屏没有 hover（focus 也不可靠），`@media (hover: none)` 里恒满。
- **已知局限（如实记）**：
  - **「运行记录」入口的默认可见度**：C21 第一版是 `opacity: 0`（不悬停就完全看不见），**真机一跑就被用户否掉**（「和背景融为一体了」）⇒ 当天改成 `0.55` 半淡常显，见下面 **C21b**。⚠️ C9 当年反对的是「入口要等**有内容**才出现」，那条**仍然成立**：入口从不消失，只是默认更轻。
  - 两半在窄面板下都可能被省略号截断（各有 `title` 兜底）。合并后一行承载的信息比原来两条**一个字不少**，但**可见宽度是共享的**。
  - 行里混着中文标签与数字，**仍不挂等宽字体**（沿用原注释的理由：会 fallback 到系统等宽中文字体，更丑）；`font-variant-numeric: tabular-nums` 只解决数字本身。
  - `.near` 现在靠**底色**而不是边框表达 —— 底色在极窄的 composer 里比边框更占眼，但它只在**接近压缩阈值**时出现，本就是要它抢眼。
- **判据 / 探针**：`probe-webview-render`（C21 落笔时 **104/104** = 原 99 + 5；⚠️ **C22 之后是 112/112** —— 这中间 C22 又把 D1 那组从"行"改指到"环"，D1 的条数没变、另加 8 条）。必改 14 条（D1 的 11 条 + C9 的 3 条）**只是换元素**，语义断言（`hidden` / `.near` / 精确文案 / 两段 `title` / `'已压缩 1 次'` 逐字相等）**一条都没放松**；新增 5 条：
  1. **只有 runs、完全没用过 usage ⇒ 行仍要显示**且右半是「本轮尚无」（C9 那条老决定）；
  2. **两半同帧共存**（usage 文案 + run 文案 + 入口按钮三者同时在行里），入口的归属按 **chat.html 原文**钉（影子不给元素连父子关系，`statusRow().children` 在那里恒为空）；
  3. **快照那一路**：两半随快照整帧重放 —— 这条专抓「两个字段名被合并成同一个」；
  4. **结构守卫**：chat.html 与 chat.js 里不再有 `usage-bar` / `runs-bar` / `usage-summary` / `runs-summary` 四个 id（防「搬一半」留死代码）；
  5. **CSS 守卫**：`.status-row` 默认规则里没有 `border` / `background`、`.status-row.near` 里仍用 warning 令牌（⚠️ C22 起这条改成"不许再有 `.status-row.near`"）、`.status-open` 不含 `display:none` / `visibility:hidden` 且**默认不透明度在 `(0, 1)` 区间内**（C21b 改的口径，原为"含 `opacity: 0`"）、悬停/聚焦那条规则把它推到 `1`、行上有一条 `:focus-within` 显形规则。
  - **变异测试 6 条全被抓红**（驱动在系统临时目录，不随仓库走；CRLF 文件上多行替换会静默空转 ⇒ 每条**先自证变异真的插进去了**）：① 把 `border`/`background` 加回 `.status-row` ② `.near` 不再给底色 ③ `.status-open` 改成 `display:none` ④ `hidden` 只算 `usageState`（丢掉 C9 那条）⑤ chat.html 留一个老 id 不删 ⑥ 快照那条改读 `data.readout`。
  - 不回归：`probe-run-inspector` **31/31**（它那条 `usage` 是 assistant/chunk 噪声帧，与 DOM 无关）。
- **F5（真机，harness，八条 —— 待跑）**：
  1. 常态：composer 里只剩「附件 chip 行（若有）+ 一条读数行 + 输入卡」—— 读数行**无边框无底色**，右侧「运行记录 ›」默认看不见；
  2. 鼠标**移入/移出**行 ⇒ 入口由半淡推到满 / 退回半淡（⚠️ **C21b 改了这条的判据**：原来要求的是"显形/隐去"，真机一跑用户就否了 —— 见 C21b），且左边的字**不重排**（宽度是预留的）；
  3. **键盘 Tab** 到它 ⇒ 同样显形（`:focus-within`），回车真的开浮层；
  4. 贴一张图 ⇒ 与那张四卡截图的对照：四条 → 三样；
  5. 制造一次中断 ⇒ 「继续」条**仍是卡**，且读数行**同时还在**（不是二选一）；
  6. 把 `hello.chat.compaction.thresholdRatio` 调低跑一轮 ⇒ `.near` 亮起来：全 composer **唯一**一次变色/给底色，且**行高不变**（不跳）。⚠️ **C22 之后这一处是那枚环、不是读数行** —— 这一条与 C22 的 F5 ⑤ 是**同一件事**，合并跑一次即可（跑的时候两处都要看：环应当亮，读数行应当**不**变）；
  7. 切到内嵌聊天 ⇒ 整行隐藏（老行为）；
  8. `media/dsh-live/` 在（react-live 画面）⇒ 读数行照样可见（它在 composer 内）。
  - ⚠️ **F5 前的准备**（这个仓库的老坑，先写在这儿免得白跑）：试验窗口要先 **Open Folder**；⑥ 要真触发得等一次**重连**让阈值生效。
- **本次不做**：不给读数行加边框/底色（那是这次改动的反面）；~~不把「运行记录」入口改成常显~~（⚠️ **这条当天就被 C21b 推翻** —— 用户真机上撞见"入口找不着"，见下）；不动附件 chip（它的「图片输入未接通」是 C20 拍板的三处联动之一）；不动 `.composer` 的 `gap`（少一层卡，间距自然就松了）；不做「读数行可折叠/可关闭」（一个能撒谎的开关比没有开关更坏，同 C20）。

#### C21b · 「运行记录 ›」入口：从「不悬停就看不见」改成「半淡常显」（2026-09-24，同日，真机反馈）
- **由来**：C21 落笔当天用户就在真机上看见 `本轮尚无　运行记录 ›`，报「**运行记录按钮只有把鼠标放在上面才能看见，和背景融为一体了**」。
  - **这不是"再显眼一点"的审美要求，是一个判据失效**：`opacity: 0` 的入口**在视觉上等于不存在**，而 C21 又特意保留了它的宽度（`.status-open` 恒占位）⇒ 行尾留着一块**空白的、什么都看不见的**位置。C9 那句「入口不等第一轮跑完」在 DOM 上做到了（入口一直在），**在屏幕上没做到**（看不见就是不在）。
  - 更要紧的是：**`opacity: 0` 那条路探针测不到**。影子不解析样式，CSS 守卫当时卡的是字面 `opacity: 0` —— 也就是说**守卫本身把这个毛病写成了正确**。凡是"判据写成字面量"的地方都有这个风险：它证明的是"写的是这个值"，不是"这个值是对的"。
- **判据（用户 2026-09-24 拍板）**：**半淡常显** —— 默认约 `0.55`（看得见，但明显比运行读数轻），鼠标停在行上 / 键盘 Tab 到时推到满。三条候选（常显 / 半淡常显 / 常显加淡底）由用户选，选中的是中道那条。
  - 为什么不选"常显"：那样它就和运行读数一样重了，C21「只有要你动手的才配抢注意力」的精神会被削掉一半。
  - 为什么不选"加淡底"：那等于给它一张（虽然很淡的）表面 —— 与"读数不画卡"直接顶。
- **落点**：[media/chat.css](../media/chat.css) 里 `.status-open` 三行数字：默认 `opacity: 0.55`（原 `0`）、悬停/聚焦那条规则推到 `1`（不变）、`@media (hover: none)` 恒 `1`（不变）。`media/chat.html` 与 `media/chat.js` **一个字节没动**（显隐从来只是 CSS 的事 —— 这正是当初选 `opacity` 而不是改 DOM 的好处：改口径不用碰逻辑）。
- **判据 / 探针**：C21 那条 CSS 守卫改了判据 —— 从"含 `opacity: 0`"改成 **"默认不透明度落在 `(0, 1)` 开区间内"**，并加一条"悬停/聚焦那条规则必须把它推到 `1`"（半淡是**默认**，不是**恒态**）。条数不变（`probe-webview-render` **112/112** —— 计数器数的是 `check()` 块，不是 `ok()` 断言）。
- **变异测试 3 条全被抓红**（驱动 `c21b-mutations.mjs` 在系统临时目录，不随仓库走）：① 默认改回 `0`（入口找不着）② 默认改成 `1`（与读数一样重）③ 悬停/聚焦不再推满（半淡成恒态）。三条都红在同一句话上（`必须「看得见但更轻」（0 < x < 1）` / `没把不透明度推到 1`），**而且红在"值不对"这条理由上，不是红在"找不到规则"上** —— 后者只能证明文件还在，前者才证明判据还在管这件事。
- **教训（写给下一次改样式的人）**：**默认值也是判据**。这次三条变异里两条是"把默认值往两边各挪一格"，都能被同一条守卫接住 —— 因为守卫卡的是**区间**而不是**字面量**。凡是有"刚好"和"太轻/太重"两端的地方，守卫就该写成区间加两条反控，别写成 `===` 某个数。

---

### C22 · 上下文占用改成配置条上的一枚小圆环（2026-09-24 实现，F5 八条待跑）
- **由来**：C21 刚把读数收成一行（`累计 ↑1.1M ↓57.8K · 上下文 76.9K / 1.0M（8%）`），用户当天接着提：**「能不能做成像 Claude Code 那样的、在模型版本选择和推理挡位之间的一个小圆环，可以表示上下文的占用比例，鼠标放在上面有完整的悬浮信息」**。
  - 这不是"再美化一下"。一行字**必须**待在 composer 里、和输入卡抢高度；一枚 16px 的环**能待在工具栏里** —— 那是它天然该在的地方（你调模型和档位时顺手就看见了），而且占用率是个**有上限的比例**，环天然表达"还剩多少"，首尾相连的两个端点（0% / 100%）也都画得出来。
- **判定（用户 2026-09-24 拍板四条）**：
  1. 读数行左半**完全由圆环取代**（本轮 / 累计 / 压缩次数**全进悬浮信息**，一个字不丢）；
  2. 圆环**纯读数、不可点**（不新开浮层、不造第二个要你点的东西）—— ⚠️ **这一条次日被 C22b 推翻**（真机看过之后用户要求"变成按钮 + 用推理/profile 相同的容器"）。保留下来的判断只有"它**不随 busy 禁用**"（见 C22b）；
  3. `.near` 警示色**搬到圆环上**（读数行不再变色）—— C21「全 composer 只有一处会变色」照旧成立，只是那一处从"行"换成"环"；而且读数行此后只剩运行读数，**再拿上下文的事去染它就说不过去了**；
  4. `stale`（读的是上次落盘的样本）**圆环变淡 + 提示里说明**。
- **落点（三个文件，`src/` 一个字节没动）**：
  - [media/chat.html](../media/chat.html)：`#status-row` 里删掉 `<span id="status-summary">`；在 `#live-model-wrap` 与 `#live-effort-wrap` **之间**插一个 `<span id="ctx-ring" role="img" tabindex="0" hidden>`，内含 16×16 的 SVG（两条 `<circle>`：`.ctx-ring-track` 底圈 + `#ctx-ring-arc` 弧，`r=6.5`、`transform="rotate(-90 8 8)"` 让它从 12 点起步）。
    - ⚠️ **SVG 必须静态写在 HTML 里**，chat.js 只 `getElementById` 再 `setAttribute('stroke-dashoffset', …)` —— 不只是习惯问题：探针的 DOM 影子**没有 `document.createElementNS`、也没有 `style.setProperty`**，动态建 SVG / 写 style 的路子在影子下根本走不通（而影子是 F5 之前唯一的验证路子）。
  - [media/chat.css](../media/chat.css)：删掉 `.status-summary` / `.status-row.near` / `.status-row.near .status-summary`；加 `.ctx-ring`（26×26，与两侧 `.tool-icon` 同宽同高以便对齐）+ `.ctx-ring-track` + `.ctx-ring-arc` + `.ctx-ring.near` + `.ctx-ring.stale`（`opacity: .45`）+ `.ctx-ring:focus-visible`。
    - **`r=6.5` ⇒ 周长 `2πr ≈ 40.84`**，这就是 `stroke-dasharray` 的值；JS 的 `CTX_RING_LEN` 必须与它相等（两处漂移 ⇒ 弧长整体错位，**画出来仍是个圆环**，只是比例不对 —— 肉眼最难发现的那种错，所以探针有一条守卫专门比这两个常量）。
    - ⚠️⚠️ **`.ctx-ring-arc` 那块里绝不许出现 `stroke-dashoffset` 这个属性**：JS 写的是**表现属性**，表现属性在 CSS 里优先级最低，这里写一个值就会把它整个盖掉、弧永远卡在同一个长度上，**而且不报错**。（判据认的是"声明"`stroke-dashoffset:`；`transition: stroke-dashoffset …` 是合法的，它只补动画、不设值。守卫按这个写。）
  - [media/chat.js](../media/chat.js)：`paintUsage()` → **`paintContextRing()`**（文案拼装**逐字保留**，整段进 `title` + 另给一句 `aria-label` 短话）；`renderStatusRow()` 缩成只剩 `statusRow.hidden = !paintRuns()`；新增入口 `renderReadouts()` = 环 + 行一起重绘，三个调用点（快照 / 切模式 / `usage`）改指它。
    - **弧长**：`ratio = clamp(usedTokens / contextWindow, 0, 1)`，`offset = LEN * (1 - ratio)`（占用 0% ⇒ 推满一整圈、一点不露；100% ⇒ 不推、满环）。**两端都夹** —— `usedTokens` 真的可能超过 `contextWindow`（分母是 DSH 的估算、分子是 provider 实测的），不夹的话 offset 变负数、弧会反着绕出去一整圈。`!(ratio > 0)` 顺带把 NaN 也吃掉（字段缺失时不该往 DOM 里写一个 `"NaN"`）。
    - **弧为 0 时把它整个 `hidden`**：`stroke-linecap: round` 在零长弧上会留下一个圆头，那在 0% 处**看起来像已经用了 5%** —— 一个 5% 的谎比不画更糟。
    - ⚠️ **两个状态类与 `title` 必须在任何一条 early return 之前重算**：第一版把 `classList.add/remove` 放在 `parts.length` 那道 early return **之后**，于是「读数本来是 near，然后来一条拼不出任何一段的样本」这条路会把环藏起来却**把 `.near` 留在身上** —— 下次亮起来带着一个没人注意到的旧警示色。**这正是 C10b 那一型的形状**，写法上抄了它的解药（先无条件重算状态，再谈显隐），并加了一条探针专门走这条路。
    - **没有 context 段时环仍然显示、只是不画弧**：因为「已压缩 N 次」是 C10 特意留在条上的**持久提醒**（压缩说明会随转写滚走，条上这段才常在），把整环藏掉会把它一起带走。
- **⚠️ 顺带逮到一颗已经躺着的真雷**：`.near` 用的 `--dsw-alias-state-**warning**-primary`，在 [media/dsh-live/dsh-live.css](../media/dsh-live/dsh-live.css) 里**根本没有定义** —— 那里只有 `--dsw-alias-state-warn-primary`（**warn，不是 warning**）。写错不报错，它一直静默退到 `--vscode-editorWarning-foreground` 兜底色，**看着"差不多对"，所以从 C10 到 C21 没人发现**（[media/chat.css](../media/chat.css) 里 `.approval-scope` 那处早就有一条注释记着这个坑，C16 还为此写了「不许出现 `-warning-`」的守卫，但没人回头查别处）。
  - **C22 只修了环上这一处**，并照 C16 那条守卫加了一条「不许写 `-warning-`、且 `warn` 那个令牌必须真在 `dsh-live.css` 里（明暗两套都要有）」。
  - ⚠️ **同款写法在 `chat.css` 里还有 8 处（`chat.css:153 / 641 / 668 / 699 / 746 / 1544 / 1835 / 1891 / 1927`），本轮一个字没改** —— 都不在 C22 的射程里，而且每一处都是**另一个功能区的视觉改动**（各有自己的 F5）。**这是本轮立下的、还没人认领的账**，见「交叉说明」。
- **判据 / 探针**：`probe-webview-render` **112/112**（原 104 + 8）。D1 那 11 条**只是换落点**（`statusSummary().textContent` → `ctxRing().title`、`statusRow` 的 `hidden`/`.near` → `ctxRing`），语义断言（`hidden` / `.near` / 精确文案 / 两段 `title` / `'已压缩 1 次'` 逐字相等 / 两个后缀并列）**一条都没放松**；只有一处放宽并有理由：`title` 是多行，所以"整段只有一处"那条改成比**第一行**逐字相等。新增 8 条：
  1. **弧长正比于占用比**（25% ⇒ offset 是 0.75 圈，别写反）+ 一条**单调性**反控（占用涨 ⇒ offset 必须变小）；
  2. **两个端点**：0% ⇒ offset 顶满**且弧 `hidden`**；100% ⇒ offset 0；超窗口 ⇒ 夹在 0；
  3. **没有 context 段 ⇒ 环还在、不画弧、`title` 里留着「已压缩 N 次」、`aria-label` 如实写「暂无数据」**；
  4. **`aria-label` 是短话**（带比例与状态，但不抄那整段多行口径、不含换行）；
  5. **结构守卫（HTML）**：环夹在 `#live-model-btn` 与 `#live-effort-btn` **之间**（比源码下标）、带 `tabindex="0"` / `role="img"` / 初始 `hidden`、**不挂** `.link-button` / `.tool-icon`、弧是绕了 `-90°` 的 `<circle>`（⚠️ **这一条被 C22b 整个翻面**：现在**必须挂** `.tool-icon .lc-knob`、且 `tabindex` 随 `<span>` 一起去掉 —— 见 C22b）；
  6. **结构守卫（CSS）**：`.ctx-ring` 里没有 `border` / `background` / `cursor`（它是读数不是按钮）、`.ctx-ring.near` 用**真令牌**且不含 `-warning-`、`chat.css` 里**不再有** `.status-row.near`（同一个警示不许在两处一起喊）、`.ctx-ring.stale` 有 `opacity`；
  7. **结构守卫（CSS）**：`stroke-dasharray` 与 JS 的 `CTX_RING_LEN` 相等，且该规则里没有 `stroke-dashoffset` **声明**；
  8. **令牌存在性守卫**：环用的三个令牌在 `dsh-live.css` 里**明暗两套都定义了**（写错不报错、只会静默退色 —— 这条正是被上面那颗雷教出来的）。
  - **变异测试 7 条全被抓红**（驱动在系统临时目录，不随仓库走）：① 弧的 offset 反过来（画出来仍是环，但比例是"还剩多少"）② 把警示色画回读数行 ③ 没有 context 时把整环藏掉 ④ 去掉 `tabindex` ⑤ 不挂 `.stale` ⑥ 把环挪出配置条 ⑦ CSS 里写回 `-warning-` 那个不存在的令牌。
  - 两条**第一次跑就静默空转**（CRLF 文件上多行替换用了 `\n`），被驱动里那条「变异真的插进去了」的自检当场抓住 —— 那条自检不是形式，它就是为这两条准备的。
  - 不回归：`probe-run-inspector` **31/31**。
- **F5（真机，harness，八条 —— 待跑）**：
  1. 环出现在**模型钮与推理档位钮之间**，26px 灰环，弧长与占用对得上；
  2. 鼠标移上去 ⇒ 完整悬浮信息（本轮 / 累计 / 上下文 / 压缩次数 / 口径说明，**一行不少**）；
  3. **Tab 到环** ⇒ 焦点环可见、`aria-label` 能被读屏念出来；
  4. 跑一轮 ⇒ 弧长随占用变长，**且过渡是平滑的而不是跳变**（`transition: stroke-dashoffset` 能不能在表现属性变化时真的跑起来，影子测不了 —— 跑不起来就删掉那行，弧直接跳，别留一条没用的声明）；
  5. 把 `hello.chat.compaction.thresholdRatio` 调低跑一轮 ⇒ **环转警告色、读数行不变色**（与 C21 的 F5 ⑥ 是同一件事，合并跑一次）；
  6. 关窗重开那条旧会话 ⇒ 环画得**淡**（`.stale`），悬浮信息里有「上次」；
  7. 切到内嵌聊天 ⇒ 配置条整体隐藏 ⇒ 环跟着不见；
  8. `media/dsh-live/` 在（react-live 画面）⇒ 环照样在（配置条在 composer 卡里）。
  - ⚠️ **F5 前的准备**（这个仓库的老坑）：试验窗口先 **Open Folder**；⑤ 要真触发得等一次**重连**让阈值生效。
- **已知局限（如实记）**：
  - ~~**纯键盘用户看不到那条多行说明**~~（⚠️ **这一条已由 C22b 修掉**：环现在点得开 —— Tab 到它按回车/空格就弹出只读浮层，那串多行说明整段在里面。C22b 记它是因为**这条局限正是"不可点"直接导致的**，而"不可点"次日就被推翻了）。
  - **一个空环既可能是 0%、也可能是"不知道"**（没有 context 段时环仍在、不画弧 —— 那是为了不丢掉「已压缩 N 次」这条持久提醒）。悬停分辨。
  - **极低占用率的弧被圆头放大**：`stroke-linecap: round` 在 8% 那种长度上会让弧看起来比实际长一点点。这是拿观感换来的，精确数字在悬浮信息里。
  - 环**只表示上下文占用**，"本轮 / 累计 token"从此**只在悬停时**可见（C21 那行字至少还占着一行宽）。这是用户 2026-09-24 明确要的取舍：常驻的只留"还剩多少上下文"，其余降为按需。
- **本次不做**：~~不给环加点击行为（用户拍板"纯读数"）~~（⚠️ **次日被 C22b 推翻** —— 见下）；不自绘 tooltip（原生 `title` 够用，代价记在已知局限里）；不做"环 + 百分比数字"的混合形态（那等于把一行字缩成两个字，宽度没省多少）；不动那 8 处 `-warning-` 老写法（各有自己的功能区和 F5，单独立项）。

#### C22b · 环变成配置条**第三个方块钮**，点开一个只读浮层（2026-09-24，同日，真机反馈）

- **由来**：C22 落笔当天用户就在真机上看见那枚环，提：**「希望"上下文占用改成配置条上的一枚小圆环"可以变成一个按钮，并且使用"推理挡位"和"profile"相同的容器，这样可以统一样式」**。
  - **这不是审美细节，是同一个工具栏里有两套外观来源**：旁边两个钮是 `div.lc-model-wrap > button.tool-icon.lc-knob + div.lc-model-menu`，而环是一个**裸 `<span>` + 自绘一整套 CSS**。同一条工具栏里两套规则，连 hover 表现都不一样（它们亮底，环不亮）。判据是"Dumbo 这一排控件应当只有一份样式定义"，与"好不好看"无关。
- **判定（用户 2026-09-24 拍板两条）**：
  1. **点它开一个只读浮层**（复用 `.lc-model-menu` 那套浮层外观，把完整读数分行列出来）。顺带**修掉 C22 的已知局限第 1 条**（键盘/读屏用户此前根本看不到那段多行说明 —— 原生 `title` 只在悬停弹、聚焦不弹）；
  2. **保留 `.near` 变警告色的判据，角上不点 `.on` 状态点** —— 那个点和变色说的是同一件事，两处一起喊就白建了（C21「跳色是稀缺资源」）。
- **落点（三个文件 + 探针；`src/` 一个字节没动）**：
  - [media/chat.html](../media/chat.html)：`<span id="ctx-ring" role="img" tabindex="0">` → `div.lc-model-wrap > button#ctx-ring.tool-icon.lc-knob.ctx-ring + div#ctx-ring-menu.lc-model-menu.ctx-readout`。**id 一个字不改**（点击绑定与探针都按它找元素）；SVG 仍旧静态写在 HTML 里；位置照旧（模型钮与推理钮**之间**）。
  - [media/chat.css](../media/chat.css)：整块**搬到 `.lc-knob.on::after` 之后**（⚠️ 不搬不行：`.tool-icon` 在 `chat.css:1094` 附近定义，**同权重看源码顺序** —— 留在原处的环色声明会被它盖掉）；删掉环自己的 `width` / `height` / `color` / `background`（**全部由 `.tool-icon` 提供**，这正是"同一个容器"的可检验形态）；**留下** `flex: none` / `svg` / 弧 / `.stale` / `:focus-visible`（焦点环从 `border-radius: 50%` 改回方块自己的 7px 圆角）；新增 `.ctx-readout .lc-model-item`（**盖掉手形光标与 hover 亮底** —— 它不可点，留着就是骗人）+ `.ctx-row-label` / `.ctx-row-body` / `.ctx-note`。
  - [media/chat.js](../media/chat.js)：`paintContextRing()` 重构成 **"一个数组派生两处"** —— `rows = [{label, body}]` + `notes`；单行文案 = `rows.map(ctxRowText).join(' · ')`（**逐字与 C22 相同**，D1 那些逐字断言一条没动），`title = [那一行].concat(notes).join('\n')`，浮层由**同一份** `rows` / `notes` 交给 `paintCtxReadout()`；新增 `toggleCtxMenu()` / `closeCtxMenu()`，并接上互斥、`document` click 的第四句、Esc 链一段、`refreshLiveConfigVisibility()` 里一句。
- **三处刻意与旁边三个钮不同**（都写进了代码注释，别顺手"对齐"）：
  1. **`refreshLiveConfigEnabled()` 不禁用它**：改模型/档位/profile 都要重启或改配置，所以忙时禁点；**看读数什么时候都该能看**，运行中恰恰是最想看它的时候；
  2. 开跑时那条"收起所有菜单"**不收它**（同上）；探针有一条专门守着这两条；
  3. `refreshLiveConfigVisibility()` 切走 harness 时**要关**（整条配置条都藏了，浮层不能留在原地）。
- **两条容易漏的**（都是老形状，探针各有一条专走这条路）：
  - **隐藏 ⇒ 先关浮层**：`paintContextRing()` 的两条早退路径（`!u` / `rows` 为空）里，`ctxRing.hidden = true` **之前**必须先 `closeCtxMenu()` —— 否则留一个**锚在隐形按钮上**的浮层（点不到、也关不掉）。与 C22 那条"两个状态类必须在早退之前"是同一型的漏法。（⚠️ **这一条已由 C22c 取消** —— 环不再被藏，那两条早退路径整条没了；幽灵浮层这件事还在，只是入口换成了"切走 harness 时整条配置条藏起来"，见 C22c）；
  - **浮层每帧整份重建（不管开没开）**：数字在流式过程中一直在涨，"开着才画"会多出一条**只在特定状态下才跑**的路 —— 正是 C10b 漏掉分母的形状。代价是每帧十来个 `textContent`，与整个转写重建比可忽略（如实记在下面的取舍里）。
- **判据 / 探针**：`probe-webview-render` **121/121**（C22 的 112 + 9 条新 `check()`；⚠️ **C22c 同日再改成 123/123**，见下）。C22 那条"**不挂** `.tool-icon`"整个**翻面**成"**必须挂** `.tool-icon .lc-knob，且与推理钮的类名只许差一个 `.ctx-ring`"（逐字比集合，不靠肉眼）；环自己的规则块里**不许写回** `width` / `height` / `color` / `background` / `border` / `cursor`；新增：
  1. **点环 ⇒ 开；再点 ⇒ 关**（钮与浮层同时挂 `.open`，与三个菜单同款）；
  2. **浮层与 `title` 同源**：把浮层的行**拼回去**必须逐字等于 `title` 第一行（⚠️ 判据是"拼回去相等"，**不是**按 ` · ` 切开数行数 —— 一段 body 里本来就可能带 ` · `：「本轮 ↑… ↓… · 缓存 97%」就是一个 body 里的分隔）；说明行同样逐行对应（一条 note 里的 `\n` 拆成多行，与 `title` 去掉第一行之后的行**一一对应**）；
  3. **浮层跟着流式帧刷新**（不是打开那一刻的快照；反控：旧数字不再留在里面）；
  4. **同一排浮层只开一个**（开环时另外三个菜单全收）；
  5. **点别处 ⇒ 关**（`document` 那条"点在外面"里第四句在）；**Esc ⇒ 关，且只关它**（身后的运行面板必须还开着 —— 层次链顺序）；
  6. **两条早退路径都要先关浮层**（⚠️ C22c 之后改成"切走 harness 时整条配置条藏起来必须关浮层"）；
  7. **busy 时照旧可点**（旁边三个钮灰掉时它不灰，且开着的浮层在开跑那一刻**不**被收走）；
  8. **CSS 守卫**：`.ctx-ring.near` 必须带 `:hover` 那一条（`.ctx-ring.near`（0,2,0）斗不过 `.tool-icon:hover:not(:disabled)`（0,3,0），少了它指针一压上去警告色就没了）、`.ctx-readout .lc-model-item` 有 `cursor: default` 且 hover 不给底色、浮层用的第四个令牌 ~~`--dsw-alias-label-dimmed`~~ **`--dsw-alias-label-tertiary`**（**C22d 同日换的** —— dimmed 在那套令牌梯里是最淡一档，暗色下只有 ≈1.5:1，见下）也进了**令牌存在性守卫**。
  - **变异测试 8 条全被抓红，且每条打红的正是它该打红的那条**（驱动在系统临时目录，不随仓库走）：① 环藏起来时不关浮层 ② `.near` 去掉 `:hover` 那条 ③ 浮层另起一套拼装（label 上多一个标点 ⇒ 与 `title` 漂移）④ `document` 那条漏掉环 ⑤ 开环时不收另外三个菜单 ⑥ 环的规则块里写回 `width` ⑦ 把环的 Esc 判据挪到运行面板之后 ⑧ 环不挂 `.tool-icon`。每条都先**自证真的插进去了**（CRLF 老坑），跑完按 **sha256** 原位还原并复跑全绿。
  - 不回归：`probe-run-inspector` **31/31**。
- **F5（真机，harness，八条 —— 待跑；与 C22 那八条合并跑一次）**：
  1. 环与旁边两个钮**同款**（26×26 方块、同圆角、hover 亮底、灰字）；
  2. 点它 ⇒ 浮层开，行列整齐，数字与悬停提示**一致**；
  3. 再点 / 点别处 / Esc **三种关法都关**；
  4. **开着浮层跑一轮** ⇒ 数字跟着涨（不冻结）；
  5. **运行中（busy）环仍可点** —— 与旁边三个钮灰掉形成对照；
  6. 调低 `hello.chat.compaction.thresholdRatio` + 重连跑一轮 ⇒ 环转警告色，**指针压上去仍是警告色**（不被 hover 的亮字盖掉）；
  7. 切到内嵌聊天（配置条整体隐藏）⇒ 环消失，且**浮层不会留在原地**；
  8. `media/dsh-live/` 在（react-live 画面）⇒ 照旧。
- **已知取舍（如实记）**：
  - **浮层每帧重建**（拿一次极小的 DOM 开销，换掉一整类"只在某个状态下才跑"的漏路 —— C10b 那一型）；
  - 它现在是**按钮**，所以 C21 那条"只有要你动手的才配一张卡/一个按钮"的分界在配置条上多了一个例外 —— 但这是**用户点名要的统一**，而且它在工具栏里、不在 composer 的读数区；
  - 浮层的行**复用 `.lc-model-item` 的排版但盖掉了手形光标与 hover 底色** —— 看着属于同一套，但不会假装能点。
- **本次不做**：不给它 `.on` 角点（用户拍板：与变色重复）；不做"点行选中/复制"这类动作（只读就是只读）；**不动**顺带发现的 C12 缺口 —— **profile 菜单不在 Esc 链里**（Esc 链是 模型 → 档位 → 审阅 → 运行 → 对照 → 历史，profile 那一段从来没写进去过）。那是一个**独立的老缺口**（C12 落地时就缺），混在这一轮里等于把 C22b 的 F5 变成 profile 功能的 F5。

#### C22c · 浮层排版重做 + **新建对话里也要有这枚环**（2026-09-24，同日，真机反馈）

- **由来**：C22b 落笔当天用户就在真机上点开了那个浮层，贴回三段渲染出来的文本，报：**「排版改一下，太丑了，另外新建对话时也要有这个按钮，只不过是"0"即可」**。两件事，一件是排版，一件是**推翻 C22 的一个决定**。
- **① 排版（"太丑"具体丑在哪）**：读数行照抄了 `.lc-model-item` 的排版，而那是**菜单项**的排版 —— 一行的短名字（`gpt-4o` / `level-3`）。读数是"一个标签 + 一句数字"，照抄的后果是：
  - 值折行（`上下文 76.9K / 1.0M（8%） · 上次` 放不进 `.lc-model-menu` 的 `max-width: 230px`），**一折行，`align-items: center` 就让标签浮在两行中间**；
  - 三个标签（本轮 / 累计 / 上下文）宽度不同、各排各的 ⇒ 三行的值**各自从不同的 x 起**，读起来是散着的三段；
  - 数字与口径说明之间**没有任何分隔**（`gap: 1px` 一路铺下来）⇒ 整块是一团同质的灰字。
  - **落点（纯 CSS，`chat.js` 与 `chat.html` 的排版一个字没动）**：`.lc-model-menu.ctx-readout` 给宽度覆盖（`min-width: min(260px, calc(100vw - 24px))` / `max-width: min(340px, calc(100vw - 24px))`，后者防侧栏拖窄时把浮层顶出面板）；读数行改 `align-items: baseline`；`.ctx-row-label` 按 `em` 定宽（3em = 三个汉字）+ 右对齐 ⇒ **值列左边缘对齐**；`.ctx-row-body` 改 `flex: 1 1 auto` + `min-width: 0` ⇒ 值在自己那一列里折行；`.lc-model-item + .ctx-note` 拉一条分隔线。
  - ⚠️ **宽度覆盖的选择器必须带 `.lc-model-menu`**：那一条在 `chat.css` 里排在**后面**，同权重靠源码顺序决胜 —— 只写 `.ctx-readout` 会被它的 150/230px 盖掉。**这与 C22b 那个 `.near:hover` 是同一条坑**，同一份文件里第二次踩，所以这次把它写进了守卫（探针按选择器原文钉）。
- **② 空态不再藏环（推翻 C22）**：C22 那版一条用量都拼不出来时 `ctxRing.hidden = true`（理由是"空环会被读成 0%"）。用户要的是"新建对话里也看得见它，只不过读作 0"。
  - **判据怎么落地**：`paintContextRing()` 里 `var u = usageState || {}`，两个早退路径删掉，空态**推一行读数**（`{label:'', body:'本轮尚无用量'}`）+ 一句说明（"第一次请求跑完就有数了"）—— 于是 `title` 第一行、浮层那一行、读屏那句**仍然是同一个来源**（谁在浮层里写死一句"尚无用量"就会被那条同源判据抓红）。
  - ⚠️ **但不写数字**：`usageState` 为空既可能是"刚新建"（那真的全是 0），也可能是"这个会话我们拼不出读数"（那是**未知**）。写成 `↑0 ↓0` 就把未知说成了 0 —— 所以**环读作 0，话说成"尚无"**。探针直接断言空态 `title` 里**不许出现 `↑`/`↓`**。
  - **删掉的东西比加上的多**：`ctxRing.hidden` 这个字段从此**没有写入者**（HTML 上那个静态 `hidden` 也去掉了），`paintContextRing()` 的返回值（原来是"有没有内容"）随之失去用途。**C22b 那条"隐藏 ⇒ 先关浮层"的守卫整条作废**（没有"回头再藏"这条路了），幽灵浮层这件事改由**唯一的收敛点**兜住：`refreshLiveConfigVisibility()` 切走 harness 时整条配置条藏起来、浮层跟着关 —— 探针那条守卫换了入口，没删。
  - ⚠️ **C22b 那条"两个状态类必须在任何早退之前重算"的规矩照旧保留**（早退没了，但那条规矩说的是"先重算，再谈画什么"）。
- **判据 / 探针**：`probe-webview-render` **121 → 123/123**（⚠️ **C22d 同日再改成 124/124**，见下）。翻面的 2 条（`D1 没有任何用量`、`C21 只有 runs` 里那句"环该藏着"）、换入口的 1 条（幽灵浮层）、新增 2 条：空态那一行**也走同一条同源判据**（浮层点得开、里面那句说明在），以及一组**排版守卫** —— 宽度覆盖的选择器原文、`min-width` 必须 **> 230px**（卡的是**关系**不是某个具体数，C21b 的教训）、基线对齐、标签 `em` 定宽 + 右对齐、值列的 `flex`/`min-width: 0`、说明行前有 `border-top`。
- **变异测试 13 条全被抓红，且每条打红的正是它该打红的那条**：空态又去藏环 / 空态写成数字 0 / 宽度覆盖只写 `.ctx-readout`（源码顺序坑）/ 读数行退回 `center` / 标签去掉定宽 / 说明行前不拉分隔线 / 切走 harness 不关浮层 / 环上写回静态 `hidden` / 浮层另起一套拼装 / 点外面漏掉环 / `.near` 去掉 `:hover` / Esc 判据挪到运行面板之后 / 环不挂 `.tool-icon`（后五条是 C22b 那批的复跑）。每条先自证真的插进去了（CRLF 老坑），跑完按 sha256 原位还原并复跑全绿。
- 不回归：`probe-run-inspector` **31/31**、`probe-context-window` **14/14**、`probe-approval-shell` **21/21**、`probe-shell-diag` **62/62**。
- **F5（真机，harness —— 与 C21/C21b/C22/C22b 那批合并跑）**：① 浮层排版（标签与值同一行、值列左边缘对齐、长值折行不折到标签底下、数字与说明之间有分隔线、面板拖窄时不顶出去）；② **新建对话**（一句话没发）⇒ 环在、弧是空的、点开写着「本轮尚无用量」；③ 发一句话跑完 ⇒ 数字接上；④ 切到内嵌聊天 ⇒ 环与浮层一起不见。
- **本次不做**：不在环上/浮层里写死数字 `0`（见上，未知与 0 是两回事 —— 真要显示"0 / 1M"得先把窗口大小从扩展侧送到 webview，那是**另一条通路**，等有人真需要再说）；不把口径说明折进可折叠区（浮层本来就短，多一层要点的东西不值）。

#### C22d · 口径说明那几行太暗（2026-09-24，同日，真机反馈）

- **由来**：用户贴回浮层里那两行说明，原话「这句话在黑色主题下有点暗，可以稍微亮一点」。
- **查出来的原因——不是"调色"，是当初选错了档**：那两行用的是 `--dsw-alias-label-dimmed`。翻 `dsh-live.css` 才看清：
  dimmed 在**明暗两套**里都是这套令牌梯的**最淡一档**（亮色 = `bluish-200` `rgb(225,229,238)`、暗色 = `bluish-750` `rgb(67,69,74)`），
  它是给"几乎不用看见的装饰"用的，不是给"安静的文字"用的。浮层底色又是 VS Code 的 `dropdown-listBackground`（暗色 ≈ `rgb(43,43,48)`）
  ⇒ 暗色下对比度 ≈**1.5:1**：等于把口径说明写成了隐形。梯子的实际顺序（明暗同序）是 `primary > secondary > tertiary > caption > dimmed`。
- **落点（一行 CSS，`chat.js` / `chat.html` 一个字没动）**：`.ctx-note` 的 color 改 `--dsw-alias-label-tertiary`（暗色 ≈6.6:1 / 亮色 ≈3.7:1），
  **层级照旧**：数字 `label-primary` > 行标签 `label-secondary` > 说明 `label-tertiary`；兜底链照旧一路退到 `--vscode-descriptionForeground`。
- **判据 / 探针**：`probe-webview-render` **123 → 124/124**，新增一条**按梯子顺序**的守卫 —— 说明那档必须**弱于**行标签、又**强于** `dimmed`，
  且 `color` 必须带兜底链。卡的是**位置**不是令牌名也不是色值（C21b 那条教训：卡字面量等于把当时那个值写成了正确）。
  顺带把 C22b 立的那条"第 4 个令牌存在性"守卫里的 `dimmed` 换成 `tertiary`。
- **变异测试 4 条全红、且各打红该红的那条断言**：说明退回 `dimmed`（→"已经落到 dimmed 那一侧"）/ 提到 `primary`（→"不比行标签那档弱 —— 层级反了"）/
  抹掉兜底链（→"说明行的 color 没有兜底"）/ 改成梯子外的令牌名（→"用了梯子之外的令牌"）。
- 不回归：`probe-run-inspector` **31/31**、`probe-context-window` **14/14**、`probe-image-attach` **49/49**、`probe-shell-diag` **62/62**。
- **F5（真机，并入 C21/C21b/C22/C22b/C22c 那批一起跑）**：暗色主题下点开浮层 ⇒ 那两行读得清、且仍明显轻于上面的数字；亮色主题下不该反过来变刺眼。
- **本次不做**：不顺手清理 `chat.css` 里另外 8 处 `--dsw-alias-state-warning-primary` 的欠账（那是另一件事，见「交叉说明」那条）；
  也不做"全文件令牌存在性 + 梯子"的扫描 —— 现在做会立刻红在那 8 处上。

---

#### C23 · 余额读数 + 模型列表改由远端驱动（2026-09-24 实现，F5 八条待跑）

- **由来（用户原话）**：用户贴了两段 Python，一段打 `GET https://api.deepseek.com/user/balance`、一段打 `GET .../models`，说
  **「这两个地址分别查询余额和模型列表，我要将其集成到对话中，我要在"API"按钮旁加一个"余额：xxx"的按钮，然后将模型列表的除自定义之外的其他模型选项改为显示模型列表里的内容」**。
  （脚本里带着明文 key。当时就把"建议去 DeepSeek 控制台吊销/轮换"说了，并且**那个值没有写进任何文件、日志或提交**。）
- **这是本仓第一次让扩展自己往外发请求**。此前 `src/` 里连一个 `fetch`、一个 `https://` 都没有 —— 所以这一轮真正的难点不是"调两个接口"，
  而是**把两件本来靠约定的事变成结构上的保证**：
  1. **key 只进请求头**。`fetchJson()` 内部拼 `Authorization: Bearer`，**返回值里没有 header** ⇒ 调用方拿不到它，也就没有"顺手把 key 拼进错误信息"的机会。
     守卫 G2 断言：去掉注释的代码里 `Authorization` / `Bearer` **恰好各出现一次**，且那个域名在 `src/` 里**只许出现在 `deepseekApi.ts`**。
  2. **服务端正文进任何文案之前先过 `sanitize()`**（抹掉 `sk-…` 片段 + 压平空白 + 截断 200 字）。**理由不是洁癖**：有的实现在报错时会把收到的
     Authorization 回显出来，而 tooltip 是给人看的、还可能被截图外传。探针里有一条**专门让假服务器把 key 回显进错误正文**的用例，
     断言最终产出的每一个字符串里都不含那个假 key。
- **四条拍板（用户 2026-09-24）**：① **点了就刷新**（重查余额、顺手重取模型列表；明细进 `title`：每币种的总额/赠送/充值 + 取数时刻）；
  ② **联网时机 = harness 连接时一次、新建对话时一次、之后每 5 分钟一次**（配好 key 后补取一次，靠既有的"存 key ⇒ 重启 live"那条路，不新造生命周期）；
  ③ 远端列表取不到（没 key / 断网 / 401 / 形状变了）⇒ **退回内置预设**，菜单照旧可用；④ 余额字面 **`余额：¥28.70`**
  （`CNY`→`¥`、`USD`→`$`；**认不出的币种写 `28.70 XYZ`，不替用户认一个币种**；多币种取第一个放按钮、全部在 `title` 里列全）。
- **落点**：
  - **新纯模块 [src/deepseekApi.ts](../src/deepseekApi.ts)（不 import `vscode`）** —— 全部判断都在这里：`parseAmount` / `parseBalance` / `parseModels` /
    `mergeModelList` / `formatAmount` / `usageFromBalance` / `describeError` / `sanitize` / `fetchJson`。体例同 `contextWindow` / `changeForecast` /
    `runInspector`：**判据必须能在扩展宿主之外加载**。这条在本模块尤其值 —— 这里面几乎每一行都是"拿不到 / 拿到怪东西时该怎么办"，
    而那些分支真机上一整年也撞不到一次 401，**撞到一次就是"余额安静地变成 —"**。
  - **`baseUrl` 与传输可注入** ⇒ 探针起一个本机假服务（`listen(0,'127.0.0.1')`）跑**同一条真实代码路径**，不联网、不用真 key。
    这也正是**选 `fetch` 而不是 `node:https` 的主要理由**：后者要测就得给探针造自签证书，然后"测的那条路"和"生产的那条路"就分家了。
  - **[src/chatViewProvider.ts](../src/chatViewProvider.ts)**：`_refreshBalance(force)` / `_applyBalance` / `_staleOrFailed` / `_applyModels` /
    `_postBalance` / `_startBalancePolling` / `_stopBalancePolling`，挂的全是**既有**的漏斗（连上 / 新会话 / `ready` / 存 key / 清 key / dispose / 视图 dispose / 切模式），
    不新造生命周期。⚠️ **状态一律用 `balance` 这一族名字**：`usage` 已经被 C3a 的每轮读数占了（`_postUsage`），
    第一版撞名撞出 TS 的 "Duplicate function implementation"。
  - **`media/chat.html` / `chat.js` / `chat.css`**：`#live-balance-btn` 紧跟在 `#live-api-btn` 之后（`link-button lc-balance`）；
    `.lc-model-menu` 补上 **`max-height: min(45vh, 320px)` + `overflow-y: auto`**（远端列表可能有十几项，而那个浮层**此前没有任何高度上限**，
    会向上一路顶出面板 —— 四个共用这个壳的浮层因此都获得滚动）；`.lc-balance` 补 `max-width` + 省略号（`.live-config-bar` 是 `flex:none` 且内部不换行，
    再加一枚带数字的钮会把整条拉宽）。
- **判据 / 探针**：
  - **新 [scripts/probe-deepseek-api.mjs](../scripts/probe-deepseek-api.mjs) 38/38**（8 组）。重点是**不猜**：`parseAmount` 对 `""` / `"abc"` / `NaN` /
    `Infinity` 一律 `undefined` **而不是 0**（`Number("")` 是 0、`Number(null)` 是 0 —— 两个都会把"没说"变成"零"）；`is_available` 不是布尔 ⇒ **未知**
    （**不当成 false**：说"账户不可用"是个指控）；`balance_infos` 非数组 ⇒ `shape`（"接口变了" ≠ "这个账户没有余额"），但数组里某一条脏 ⇒ **只跳过那一条**。
    HTTP 那一组把 200/401/403/404/429/500/418/畸形 JSON/**服务器永远不回 ⇒ `timeout`**/`127.0.0.1:1` ⇒ `network`/**没有传输层 ⇒ `unsupported` 降级**全覆盖
    （最后那条不是假想：**系统 node 16 就没有全局 `fetch`**，只有便携运行时里的 node 24 有 —— 探针的第一条就是钉这个前提）。
  - **`probe-webview-render` 124 → 132/132**：四档读数**逐字**上屏（含 `title`）、陈旧档**保留数值**、点一下恰好一条 `refresh-balance` 且**只带 `type` 一个键**、
    **busy 时不禁用且点了照发**、模型菜单的行来自远端列表（远端 id 故意**不带** `deepseek-` 前缀 —— 免得断言其实吃的是预设），外加 HTML/CSS 结构与**关系型**守卫。
  - **9 条变异全红，且每条打红的正是它该打红的那条**：金额解析一把梭 / 未知币种替用户认 `¥` / 失败时把上次数值擦成 `—` / `sanitize` 拆掉 /
    超时归成 `network` / 远端与预设取并集 / 菜单去掉高度上限 / busy 时禁用余额钮 / webview 再拼一次「余额：」前缀。
- **三条闸（这一轮唯一"自己会跑"的东西就是那个 5 分钟定时器）**：① **没 key ⇒ 一个请求都不发**；② **上一发没回来 ⇒ 不叠发**（`_balanceInFlight`）；
  ③ **dispose / 视图 dispose / 切走模式 ⇒ 停表**。**缺任何一条就是"后台静默发请求"**，而那种毛病在真机上表现为"偶发的、说不清哪来的网络活动"。
- **一处刻意的不一致**：余额钮**不随 busy 禁用**（旁边模型/档位/API 都灰掉时它不灰）。它只是读数，而**看读数什么时候都该能看** ——
  与 C22b 那枚环同一条判断。探针为此专门有一条**对照**：busy 时 `live-api-btn` 是禁用的、它**不是**，且点下去照发。
- **被推翻/纠正的预设（本轮）**：
  - **「tsconfig 的 lib 只有 ES2022、没有 DOM，所以用不了 `fetch`」** —— 只对一半。类型上确实不存在，但那不构成"用不了"：
    按用到的部分**自己声明**最小形状（`MinimalResponse` / `MinimalRequestInit` / `FetchLike`），再从 `globalThis` 上取，就既不用把 DOM
    或 undici 的类型拉进来（那等于给一个"零依赖"的仓悄悄加一层类型依赖），也不损失什么。**`.call(globalThis, …)` 那一层 wrapper 不是多余的**：
    浏览器里把 `fetch` 摘下来单独调用会 `Illegal invocation`。
  - **「`??` 能表达"没有传输层"」** —— 不能。`opts.fetchImpl ?? hostFetch()` 把 `null` 与 `undefined` 混成一个，
    于是**探针里那条"验 `unsupported` 降级"的用例会静默地走宿主的 fetch**、永远绿。改成显式三元：`undefined` = "用宿主的"、`null` = "这里没有传输层"。
  - **「探针的 `check()` 能验异步」** —— 不能，而且**是最坏的一种不能**：`check(fn())` 拿到一个 Promise，而 Promise **永远为真值** ⇒ 断言静默变绿。
    已拆成 `check` / `checkAsync`，且 `check` 拿到 thenable 会**当场抛**（"异步用例请用 checkAsync"）。
  - **「错误信息里带上 url 和响应正文更好排查」** —— 反了。那正是 key 可能泄漏的地方（回显），也是把 tooltip 变成一屏 HTML 的地方。
    `describeError()` 只出人话（11 句，各带状态码），正文单独过 `sanitize()` 且只在 `detail` 里。
- **本次不做**：不做手动刷新间隔的设置项（用户拍板的是固定 5 分钟）；不做余额历史 / 曲线；不做多账号 / 多 key；不做"把费用估算接上余额"
  （那是 C3b，wire 无 prompt-cancel 那条限制不变）；不给用户 `baseUrl` 配置入口（**目标写死**，想改只能改代码）。
- **不回归**：`probe-run-inspector` **31/31**、`probe-context-window` **14/14**、`probe-agent-profile` **32/32**、`probe-approval-roundtrip` **24/24**。
- **F5（真机，harness；八条 —— 待跑）**：① 余额钮出现且是真数字（`¥xx.xx`）；② 点一下 ⇒ 数字刷新、`title` 里的时刻跟着变；
  ③ 跑动中（busy）⇒ 它**仍可点**（对照旁边三个钮灰掉）；④ 模型菜单是**远端列表**（不是那三个预设）、长列表在菜单内滚动且不顶出面板、「自定义模型…」仍在末尾；
  ⑤ 把 key 清掉 ⇒ 翻「余额：—」、菜单退回预设；⑥ 断网 ⇒ 保留上一次的好值 + `title` 写明最近一次失败；⑦ 新建对话 ⇒ 立刻重取一次；
  ⑧ 关面板 / 切到内嵌聊天 ⇒ 5 分钟的表停下（扩展宿主日志或 `netstat` 侧证）。
...
- **⚠️ 一笔未认领的账：`--dsw-alias-state-warning-primary` 在 `chat.css` 里还有 8 处（2026-09-24 C22 发现）**。这个令牌名**在 `dsh-live.css` 里根本没有定义**（那里只有 `warn`），所以每一处都在静默退到 `--vscode-editorWarning-foreground` 兜底色 —— **不报错、看起来也对**，所以 C10 → C21 一路没人发现，直到 C22 把 `.near` 从读数行搬到环上才撞见。
  - 剩余 8 处：`chat.css:153 / 641 / 668 / 699 / 746 / 1544 / 1835 / 1891 / 1927`（`chat.css:934` 与 `:1558` 用的是**对的** `warn` —— 后者正是 `.approval-scope`，C16 当年专门为此加了守卫）。
  - **C22 只修了环上那一处**，没顺手改这 8 处：每一处都落在**不同功能区**（审批条 / 审阅条 / 工具卡 / 空态…），改色必须各自过一遍真机，混在一个改动里做等于把 C22 的 F5 变成 9 个功能的 F5。
  - **要修的话**：逐处替换成 `--dsw-alias-state-warn-primary`，然后照 C22 第 8 条那样加一条**令牌存在性**守卫（`dsh-live.css` 里明暗两套都得有）—— 守卫比人眼可靠：这个错"看起来是对的"。
  - **教训（比这 9 行字重要）**：**兜底值会把写错的令牌名伪装成写对的。**凡是令牌写了 fallback，写错就只是"颜色差一点点"，测试和肉眼都抓不住 —— 所以要么有**存在性守卫**，要么那个令牌名值得有个探针钉着。
