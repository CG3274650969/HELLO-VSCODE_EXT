...
| C11 | 会话级推理档位（reasoningEffort） | P1 | **原判「受限：需 runtime 先支持」已推翻** —— 机制早就在（provider 按请求解析档位、`agent/request` 瀑布的返回值就是请求 config），缺的只是入口。**已实现：自检 25/25 全绿；F5 的 ① ② ⑤ 已通过；④ 揪出一个真 bug（换会话不重播配置条，已修 `a7e5bab` + 加了结构守卫）待复验；③ 还差「同进程两个会话各设不同档位」**（热切 + 真会话级；DSH 侧一行不改、用户配置一行不碰，纯追加一个我们自己的插件块） | [~] |
...
- **只能真机 F5 盖住**：
  ...
  - ⬜ ③ **会话级的真判据**：两个会话各设不同档位、各跑一轮 ⇒ 两份日志各对各的（进程是同一个）。目前只证到「设过档位的那份是 low、没设过的两份是 max」。
  - ⚠️ ④ 重载窗口后档位还在（会话字段落盘），且不需要再重连（刚起就连、块一开始就在）。**F5 当场揪出一个真 bug，已修（`a7e5bab`）—— 待复验**：重载 → 从「历史」点回那条设过 `low` 的会话，菜单应显示 `推理 · low` 且不重连。⚠️ 重载后落在**全新空会话**上显示「跟随配置」是**对的**（C5 的设计：要接着聊得点回原会话），别把它当 bug。
