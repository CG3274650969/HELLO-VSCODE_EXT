...
- **判据 / 探针**：`probe-webview-render` **121/121**（C22 的 112 + 9 条新 `check()`；⚠️ **C22c 同日再改成 123/123**，见下）。C22 那条"**不挂** `.tool-icon`"整个**翻面**成"**必须挂** `.tool-icon .lc-knob，且与推理钮的类名只许差一个 `.ctx-ring`"（逐字比集合，不靠肉眼）；环自己的规则块里**不许写回** `width` / `height` / `color` / `background` / `border` / `cursor`；新增：
  ...
  8. **CSS 守卫**：`.ctx-ring.near` 必须带 `:hover` 那一条（`.ctx-ring.near`（0,2,0）斗不过 `.tool-icon:hover:not(:disabled)`（0,3,0），少了它指针一压上去警告色就没了）、`.ctx-readout .lc-model-item` 有 `cursor: default` 且 hover 不给底色、浮层用的第四个令牌 `--dsw-alias-label-dimmed` 也进了**令牌存在性守卫**。
...
- **判据 / 探针**：`probe-webview-render` **121 → 123/123**。翻面的 2 条（`D1 没有任何用量`、`C21 只有 runs` 里那句"环该藏着"）、换入口的 1 条（幽灵浮层）、新增 2 条：空态那一行**也走同一条同源判据**（浮层点得开、里面那句说明在），以及一组**排版守卫** —— 宽度覆盖的选择器原文、`min-width` 必须 **> 230px**（卡的是**关系**不是某个具体数，C21b 的教训）、基线对齐、标签 `em` 定宽 + 右对齐、值列的 `flex`/`min-width: 0`、说明行前有 `border-top`。
