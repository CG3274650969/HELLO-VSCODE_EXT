...
| `scripts/probe-webview-render.mjs` | Self-check for `media/chat.js` rendering, run in Node behind a minimal DOM shim (the only way to exercise that code before F5): the usage bar's `near` / `stale` / `compacted` states and its `.near` class, the transcript fold (head folded, correct count, expand restores everything, reset on session switch), the same fold replayed over the **longest real stored session** (99 messages / 56 tool cards), plus a small C9 runs-bar/panel regression sample. It has already caught two real bugs of the “a shadow that lies is worse than no shadow” kind (same; no VS Code, no API key) |
