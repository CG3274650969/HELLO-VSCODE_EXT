...
export interface StoredSession {
  ...
  /** C20：**本会话见过图片输入通路**（运行时把 `read_image` 注册出来了 ⟺ 挂上了附件仓库）。
   *
   *  为什么非落盘不可：理由与 `contextWindow` 逐字相同 —— `request/header` 也**只在会话开头 /
   *  变化 / 恢复时写一条**，续聊与重连都见不到新的；而 chip、导出行、系统提示词那段三处都要按它
   *  渲染。只放内存的话，重载窗口或切回旧会话时那句标签就退回「未接通」，明明是接通的。
   *
   *  ⚠️ **缺省 = 关**（fail-closed）：没有证据就是今天的行为。绝不允许「没读到」被渲染成「已接通」。
   *  旧数据没有本字段 → 向后兼容。 */
  imageRead?: boolean;
}
